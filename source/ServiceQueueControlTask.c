...
void ServiceQueueControlTask(void *param_struct)
{
  ...
  int current_acel = 2;
    ...
    int time_to_spend_in_acel = 0;
    int  time_to_spend_in_cruise = 0;
                ...
                switch(current_working_req)
                {
                    case CallToP2InsideCar:
                        if (new_service == true)
                        {
                            ...
                            if (current_floor == 0)
                            { 
                                cruise_distance = acel_and_decel_distance - DISTANCE_FROM_GND_TO_P2;
                                time_to_spend_in_acel = current_max_speed/current_acel;
                                time_to_spend_in_cruise = cruise_distance/current_max_speed;
                            } 
                            else if (current_floor == 1)
                            {
                                cruise_distance = acel_and_decel_distance - DISTNACE_FROM_GND_TO_P1;
                                time_to_spend_in_acel = current_max_speed/current_acel;
                                time_to_spend_in_cruise = cruise_distance/current_max_speed;
                            }
                            else if (current_floor == 2)
                            {
                                 // Tell the motor to sit tight.
                            ...
                            motor_message_to_send.m_time_to_spend_in_accel = time_to_spend_in_acel;
                            motor_message_to_send.m_time_to_spend_in_cruise = time_to_spend_in_cruise;
                            motor_message_to_send.m_time_to_spend_in_decel = time_to_spend_in_acel;
                    ...
                    case CallToGNDInsideCar:
                        if (new_service == true)
                        {
                            ...
                            // Get the cruise distance for our current path.
                            if (current_floor == 0)
                            {  // ... We are already on this floor silly.
                                time_to_spend_in_acel = 0;
                                 time_to_spend_in_cruise = 0;
                            } 
                            else if (current_floor == 1)
                            ...
                            strcpy(motor_message_to_send.state, motor_msg_default);
                            motor_message_to_send.m_time_to_spend_in_accel =  time_to_spend_in_acel;
                            motor_message_to_send.m_time_to_spend_in_cruise = cruise_distance/current_max_speed;
                            motor_message_to_send.m_time_to_spend_in_decel = time_to_spend_in_cruise;
                            motor_message_to_send.m_emer_flag = false;
                            motor_message_to_send.m_start = false;
